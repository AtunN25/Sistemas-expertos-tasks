...
class Nodo():
    def __init__(self, estado, padre, accion,valorHeuristico):
        self.estado = estado
        self.padre = padre
        self.accion = accion
...
class FronteraPrioridad():
    ...
    def add(self, nodo):
        ...
        self.frontera.sort(key=lambda x: x.valorHeuristico)
...
class Laberinto():
    ...
    def solve(self):
        ...
        inicio = Nodo(estado=self.inicio, padre=None, accion=None,valorHeuristico=Heuristico(self.inicio, self.meta))
        ...
        while True:
            ...
            for accion, estado in self.vecinos(nodo.estado):
                if not frontera.contiene_estado(estado) and estado not in self.explorado:
                    ...
                    child = Nodo(estado=estado, padre=nodo, accion=accion,valorHeuristico=nuevo_valor_heuristico)
                    frontera.add(child)
                    self.heuristicas[estado] = nuevo_valor_heuristico# Guardar valor en el diccionario
